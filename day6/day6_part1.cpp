...
int main()
{
    ...
    for (int input_line_idx = 0; input_line_idx < line_count; ++input_line_idx)
    {
        ...
        if (input_str.at(0) != '*' && input_str.at(0) != '+')
        {
            ...
            line.m_numbers = ExtractNumbers(input_str);
        }
        else
        {
            ...
            for (int operation_idx = 0; operation_idx < (int)operations.size(); ++operation_idx)
            {
                ...
                {
                    group.m_numbers.emplace_back(lines.at(line_idx).m_numbers.at(operation_idx));
                }
            }
        }
    }
    ...
    for (int group_idx = 0; group_idx < (int)groups.size(); ++group_idx)
    {
        ...
        {
            uint64_t total{0};
            for (int num_idx = 0; num_idx < (int)g.m_numbers.size(); ++num_idx)
            {
                total += g.m_numbers.at(num_idx);
            }
            grand_total += total;
        }
